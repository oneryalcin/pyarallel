...
- Both timeouts exist here: `timeout` is total wall-clock (same contract as sync), `task_timeout` is per-task via `asyncio.wait_for` — the sync API deliberately has no per-task timeout (threads can't be cancelled)
- On errors and timeouts, in-flight tasks are cancelled and awaited — proper cleanup, nothing left running on the loop
...
When `items` is unsized (for example a generator), input consumption is lazy — one window ahead, never materialized — and `total` is the number of items *admitted* so far, growing as the run progresses: a percentage over it is meaningless. Pass a sized input for a real total.
...
Async tasks, unlike threads, are genuinely cancellable — once the generator is closed, in-flight work stops.
