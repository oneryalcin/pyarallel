...
Real APIs speak 429 + `Retry-After`. `retry_if` decides retryability from the exception *instance*, and `wait_from` extracts the server-mandated wait — it replaces the backoff delay with no jitter and no `max_delay` cap, because the server knows better than we do:
