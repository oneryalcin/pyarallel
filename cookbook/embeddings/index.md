...
- **`Retry.for_http()`** — when the API returns 429 with `Retry-After` (numeric or HTTP-date form), that wait replaces the backoff *and* pauses the shared limiter, so one throttled task slows the whole pool instead of every worker discovering the limit separately. `APIConnectionError` carries no status; the type filter decides for it.
